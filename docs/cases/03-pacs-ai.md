...
- **指标序列**：`2024.06 · 1.43 颗` ➔ `2025.06 · 3.46 颗`（后台同期记录，平均每张小牙片龋齿发现数）
- **时间线锚点**：AI 功能约于 `2025.11` 上线。
- **严格口径说明**：两者均早于 AI 上线时间，属于系统规范化后后台记录的客观提升，不能归因于 AI；目前暂无 AI 上线后的结果数据。
...
- **2024.06**：平均每张小牙片龋齿发现数 **`1.43 颗`**（后台历史记录）
- **2025.06**：平均每张小牙片龋齿发现数 **`3.46 颗`**（后台同期记录）
- **2025.11**：AI 辅助功能正式灰度上线。

### 6.2 严谨归因说明 (Rigor & Attribution Boundary)
> **重要说明**：`1.43` 与 `3.46` 均早于 AI 上线时间，这一增长主要反映了 **V1 结构化读片规范建立后，医生记录完整度的自然提升**，不能归因于 AI 模型的贡献。在严谨的医疗产品定义中，不能将“平均发现数”混淆为医学金标准下的“检出率”。
