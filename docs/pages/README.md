...
2. **内容定稿**：在 `pages/<slug>/content.md` 编写 0-8 九段式结构的中英双语母稿。
3. **设计归档**：在 `pages/<slug>/design.md` 记录视觉与交互规范、设计稿链接与关键截图。
4. **全网注册**：在本文注册表与 `docs/site/content-map.md` 登记，并按需同步至 `src/data/projects.js`。

---

## 三、 案例内容母稿结构标准（content.md）

每个 `content.md` 严格遵循以下 0-8 九段式结构（2026-08-19 由原 7 步证据链演进定稿）：

0. **元数据与个人定位 (Metadata & Scope)**：页面 Slug、双语标题、标签、个人职责与交付边界、核心命题。
1. **首屏与核心指标 (Hero & Key Metrics)**：首屏大图/视觉锚点 + 瞬时识别指标卡。
2. **现场与代价 (Context & Tension)**：多角色业务断点与未被系统承接的现实损失。
3. **根因分析与设计命题 (Diagnosis & Mission)**：穿透表面痛点，抽象系统缺陷并下达一句话设计命题。
4. **核心约束与边界 (Constraints & Scope)**：技术、组织、合规与时间边界。
5. **关键机制与产品决策 (Mechanism & Product Decisions)**：统一子节骨架——`5.1 核心解决方案`、`5.2 核心决策与权衡`、`5.3+ 机制细节（项目特有，可多节，编号顺延）`、`5.4+ 设计体系与 UX 手艺（置后）`。
6. **业务结果与指标口径 (Business Impact & Rigor)**：核心指标、统计周期、样本人群、数据来源、归因边界。
7. **复盘反思与未解局限 (Retrospective)**：真实意外收获、妥协代价与未来演进方向。
8. **视觉资产与代码映射 (Assets & Code Mapping)**：对应图片路径、数据源文件、页面组件与样式模块。

> 统一规则：第 5 章「共性子节（解决方案 / 决策权衡 / 设计体系）命名固定、顺序固定」，特性子节（机制细节）插入中间并顺延编号；第 0-4、6-8 章为全局固定骨架，不得删改顺序。
